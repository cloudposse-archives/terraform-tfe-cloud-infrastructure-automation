...
| config\_name | Configuration File Name of the workspace | `any` | n/a | yes |
...
| runtriggers | Map of Run Triggers for Terraform Enterprise | `map` | `{}` | no |
| stage | Stage, e.g. 'prod', 'staging', 'dev', OR 'source', 'build', 'test', 'deploy', 'release' | `string` | `null` | no |
| tags | Additional tags (e.g. `map('BusinessUnit','XYZ')` | `map(string)` | `{}` | no |
| token | TFE token | `any` | n/a | yes |
| variables | Map of environment or Terraform variables to define in the workspace. | `map` | `{}` | no |
| vcs\_repo | The VCS repository to configure. | `map(string)` | `{}` | no |
| workspace\_id | ID of TFE Workspace | `any` | n/a | yes |
| workspace\_ids | Map of All TFE Workspace IDs | `any` | n/a | yes |
